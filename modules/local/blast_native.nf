process NATIVE_BLAST {
    ...
    publishDir params.outdir, mode: 'copy', overwrite: true,
        saveAs: { name -> name in ['taxonomy_blast_report.tsv', 'taxonomy_blast_changed.tsv', 'taxonomy_blast_evidence.tsv'] ? name : null }
    input:
    path repseq
    path taxonomy
    output:
    ...
    python3 ${scripts}/reconcile_qiime_blast_species.py --qiime-normalized normalized/taxonomy_normalized.tsv --blast-taxonomy blast_taxonomy/blast_taxonomy.tsv --output-dir final_taxonomy --max-evalue ${params.blast_max_evalue} --min-pident ${params.blast_min_pident} --min-qcovus ${params.blast_min_qcovus} --reconcile-mode ${params.blast_reconcile_mode} --lineage-policy ${params.blast_lineage_policy} --species-min-pident ${params.blast_species_min_pident} --species-min-qcovus ${params.blast_species_min_qcovus} --species-max-evalue ${params.blast_species_max_evalue}
